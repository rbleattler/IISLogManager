...
public class CommandProcessor {
		...
		if (config.TargetSites != null) {
			...
			if (config.TargetSites != null)
				foreach (var site in config.TargetSites) {
					...
					ProcessSite(config, ref targetSite);
					site.Dispose();
				}
		}
	}
				...
				switch (config.OutputMode) {
					...
					case OutputMode.LocalDb:
						...
						var iisLogManagerContext = new IISLogManagerContext(
							...
							config.Settings?.ExcludeFields.Split(',')
						);
								...
								await Parallel.ForEachAsync(source: site.LogFilePaths, parallelOptions,
									async (p, cancellationToken) => {
										...
										var innerContext = new IISLogManagerContext(
											...
											refConfig.Settings?.ExcludeFields.Split(',')
										);
	...
	private void ProcessSite(CommandConfiguration config, ref SiteObject siteReference) {
	...
	private void ProcessLocalDbSite(CommandConfiguration config, ref SiteObject siteReference) {
		var site = siteReference;
		var iisLogManagerContext = new IISLogManagerContext(
			config.DatabaseProvider ?? DatabaseProvider.Sqlite,
			config.ConnectionString ?? throw new ConnectionStringNotSpecifiedException(),
			config.Settings?.TableName ?? "IISLogs",
			config.Settings?.ExcludeFields?.Split(',')
		);
		iisLogManagerContext.Database.EnsureCreated();
			...
			ctx => {
				...
				site.LogFilePaths.ForEach(p => {
					...
					if (totalLines > maxProcessCount) {
						...
						while (processedCount < totalLines) {
							int remaining = totalLines - processedCount;
							var processMax = remaining >= maxProcessCount;
							...
							if (site.Logs != null) {
								...
								site.Logs.Clear();
								GC.Collect();
								fileSizeProcessed += fileSizeMegs / maxProcessCount;
								ctx.Refresh();
							}
						}
					} else {
